...
Ship.prototype.fire = function () {
  if (this.fired === false && this.invuln === false) {
    this.fired = true
    setTimeout(function () {this.fired = false}.bind(this), this.firespeed)
    ...
    if (this.trishot) {
      var new_bullet1 = new Bullet(this.angle-1, [this.xpos, this.ypos], this.game)
      this.game.bullets.push(new_bullet1)
      setTimeout(function() {
        var idx = this.game.bullets.indexOf(new_bullet1)
        if (idx > 0) {
          this.game.removeBullet(idx)
        }
      }.bind(this), 1500)
      var new_bullet2 = new Bullet(this.angle+1, [this.xpos, this.ypos], this.game)
      this.game.bullets.push(new_bullet2)
      setTimeout(function() {
        var idx = this.game.bullets.indexOf(new_bullet2)
        if (idx > 0) {
          this.game.removeBullet(idx)
        }
      }.bind(this), 1500)
